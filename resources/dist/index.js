...
(function init() {
  ...
  var addPopupEventHandlers = function addPopupEventHandlers(target) {
    ...
    target.addEventListener('keyup', function (event) {
      ...
      var _clientRects = _slicedToArray(clientRects, 1),
          firstClientRect = _clientRects[0];
    ...
    target.addEventListener('click', function (event) {
      ...
      if (mwTitle) {
        if (_preview_model__WEBPACK_IMPORTED_MODULE_19__["previewTypes"].TYPE_PAGE === Object(_preview_model__WEBPACK_IMPORTED_MODULE_19__["getPreviewType"])(target, mw.config, mwTitle)) {
...
function _show(preview, event, $link, behavior, token, container, dir) {
  var pageX = event.clientX + event.pageXOffset;
  var pageY = event.clientY + event.pageYOffset;
...
function createLayout(isPreviewTall, eventData, linkData, windowData, pointerSpaceSize, dir) {
  var flippedX = false,
      flippedY = false,
  ...
  getClosestYPosition(eventData.pageY - windowData.scrollTop, linkData.clientRects, false) + windowData.scrollTop + pointerSpaceSize : // Position according to link position or size
