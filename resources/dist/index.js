...
(function init() {
  ...
  var addPopupEventHandlers = function addPopupEventHandlers(target) {
    ...
    var getNearestClientRect = function getNearestClientRect(rects, y) {
      return Array.from(rects).reduce(function (result, rect) {
...
/*!****************************!*\
  !*** ./src/ui/renderer.js ***!
  \****************************/
/*! exports provided: createPointerMasks, init, render, createPreviewWithType, show, bindBehavior, hide, createLayout, getClasses, layoutPreview, setThumbnailClipPath, getThumbnailClipPathID */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
function _show(preview, event, $link, behavior, token, container, dir) {
  var layout = createLayout(preview.isTall, event, pointerSize, dir);
  ...
  previewElement.removeAttribute('aria-hidden'); // Trigger fading effect for reference previews after the popup has been rendered
  ...
  return Promise.race([Object(_wait__WEBPACK_IMPORTED_MODULE_0__["default"])(200), new Promise(function () {
    return bindBehavior(preview, behavior);
  })]);
}
/**
...
function _hide(preview) {
  ...
  var fadeOutClass = fadeInClass === 'mwe-popups-fade-in-up' ? 'mwe-popups-fade-out-down' : 'mwe-popups-fade-out-up'; // Classes documented above
...
function createLayout(isLandscape, eventData, pointerSpaceSize, dir) {
  var flippedX = eventData.clientX > eventData.innerWidth / 2;
  var flippedY = eventData.clientY > eventData.innerHeight / 2;
  var offsetCorrection = eventData.eventType === 'mouse' ? 18 : 0;
  var clientOffset = {
    left: eventData.clientX + (flippedX ? (eventData.eventType === 'key' ? eventData.clientRect.width : 0) + offsetCorrection - (isLandscape ? landscapePopupWidth : portraitPopupWidth) : -offsetCorrection),
    top: flippedY ? eventData.clientRect.top - pointerSpaceSize : eventData.clientRect.bottom + pointerSpaceSize
  };
  return {
    offset: {
...
function layoutPreview(preview, layout, classes, predefinedLandscapeImageHeight, pointerSpaceSize) {
  ...
  if (!flippedY && !isTall && hasThumbnail && thumbnail.height < predefinedLandscapeImageHeight) {
    popup.find('.mwe-popups-extract').css('margin-top', thumbnail.height - pointerSpaceSize);
  } // eslint-disable-next-line mediawiki/class-doc
...
function renderPopup(type, html) {
  ...
  return $($.parseHTML("\n\t<div class='mwe-popups mwe-popups-type-".concat(type, "' aria-hidden>\n\t\t<div class='mwe-popups-container'>").concat(html, "</div>\n\t</div>\n\t").trim()));
}
...
function createThumbnailElement(className, url, x, y, thumbnailWidth, thumbnailHeight, width, height) {
  ...
  $thumbnailSVGImage[0].setAttributeNS(nsXlink, 'href', url); // eslint-disable-next-line mediawiki/class-doc
